...
function escape_label(label: string) {
  return label
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
...
function indent_stack(stack: string) {
  return escape_label(
    stack
      .split("\n")
...
function error_to_string(error: Error) {
...
export class ErrorPage extends Gtk.Box {
  static {
    GObject.registerClass({
      GTypeName: "ErrorPage",
      Template: "resource:///org/example/TypescriptTemplate/pages/error.ui",
      InternalChildren: [
        "status",
        "more",
        "text_view",
